...
NUM_ROUNDS      = 20
LOCAL_EPOCHS    = 1
CLIENT_FRACTION = 0.1    # 10 clients per round
CLIENT_LR       = 0.02
SERVER_LR       = 0.001  # important: much smaller than 1.0 for FedOpt
BATCH_SIZE      = 32

global_model = MNIST_MLP().to(device)
server_optimizer = optim.Adam(global_model.parameters(), lr=SERVER_LR)

print("\n=== Starting FedOpt Training ===")
for rnd in range(1, NUM_ROUNDS + 1):
    t0 = time.time()
    global_weights = global_model.state_dict()

    total_samples = 0
    sum_deltas    = [torch.zeros_like(w) for w in global_weights.values()]

    # 2a) Sample a random subset of clients
    ...
    for cid in selected_clients:
        ...
        # 2b.iii) Compute delta = (local_weights – global_weights)
        ...
        for i, key in enumerate(global_weights.keys()):
            delta_i = local_weights[key].to(device) - global_weights[key].to(device)
            sum_deltas[i] += delta_i * num_samples
        total_samples += num_samples

    # 2c) Average the deltas
    avg_deltas = [sum_deltas[i] / total_samples for i in range(len(sum_deltas))]

    # 2d) Apply server‐side optimizer: manually set each param.grad = -avg_delta
    server_optimizer.zero_grad()
    for (param_name, param_tensor), delta in zip(global_model.named_parameters(), avg_deltas):
        # Assign negative of average delta to param.grad
        param_tensor.grad = -delta.clone().to(device)

    server_optimizer.step()

    # 2e) Evaluate on the global test set
...
print("=== FedOpt Training Complete ===")
