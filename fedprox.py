...
for rnd in range(1, NUM_ROUNDS + 1):
    ...
    for cid in selected_clients:
        ...
        # 2b.ii) Local training with proximal penalty
        local_model.train()
        for _ in range(LOCAL_EPOCHS):
            for Xb, yb in client_train_loaders[cid]:
                ...
                # proximal term: sum ||w_local - w_global||^2 over all layers
                prox_loss = 0.0
                for name, param in local_model.named_parameters():
                    prox_loss += torch.norm(param - global_weights[name].to(device))**2
